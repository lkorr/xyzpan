...
public:
    ...
    {
        // When roll is locked, dampen horizontal input near pitch ±90
        // and clamp pitch to ±88.5° to avoid the gimbal singularity.
        float dx = screenDx;
        if (rollLocked_) {
            constexpr float kDeg2Rad = 3.14159265358979323846f / 180.0f;
            const float cosP = std::cos(prevRPY_.pitchDeg * kDeg2Rad);
            constexpr float kMinScale = 0.05f;
            dx *= std::max(cosP * cosP, kMinScale);
        }
        ...
        const Quat qYaw   = quatFromAxisAngle(up.x,    up.y,    up.z,    -dx * sensitivity);
        ...
        // If roll is locked, restore the pre-delta roll value and
        // clamp pitch to ±88.5° to stay away from the gimbal singularity.
        if (rollLocked_) {
            prevRPY_.rollDeg = lockedRollDeg_;
            prevRPY_.pitchDeg = std::clamp(prevRPY_.pitchDeg, -88.5f, 88.5f);
            q_ = rpyToQuat(prevRPY_);
        }
    }
    ...
    void applyRollDelta(float rollDeltaDeg)
    {
        ...
        prevRPY_ = quatToRPY(q_, prevRPY_);
        if (rollLocked_) lockedRollDeg_ = prevRPY_.rollDeg;
    }
    ...
    void syncFromRPY(float yawDeg, float pitchDeg, float rollDeg)
    {
        ...
        q_ = rpyToQuat(rpy);
        prevRPY_ = rpy;
        if (rollLocked_) lockedRollDeg_ = rollDeg;
    }
    ...
    // Roll lock: when enabled, mouse movement cannot change roll.
    // Q/E and knob edits still change roll (and update the locked value).
    void setRollLocked(bool locked)
    {
        rollLocked_ = locked;
        if (locked)
            lockedRollDeg_ = prevRPY_.rollDeg;
    }
    bool isRollLocked() const { return rollLocked_; }
